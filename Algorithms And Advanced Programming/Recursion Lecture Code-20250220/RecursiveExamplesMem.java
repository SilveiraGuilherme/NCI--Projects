public class RecursiveExamplesMem {
    ...
    public static void main(String[] args) {
        ...
        int fibNumMem = fibRecursiveMem(n);
        int fibNumRecursive = fibRecursive(n);

        System.out.println("Memoization:" + fibNumMem);
        System.out.println("Recursive:" + fibNumRecursive);
    }
    ...
    public static int fibRecursiveMem(int n){ // this method uses memoization

        if(n==1 || n==2){
            return 1;
        }
        // if we already calculated this value then we lookup the value
        else if(cache[n-1] != 0){
            return cache[n-1];
        
        }
        else{
            cache[n-1] =  fibRecursiveMem(n-1) + fibRecursiveMem(n-2);
            return cache[n-1];
        }
